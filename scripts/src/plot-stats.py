...
def plot_tp_sizes(data, output_dir):
    ...
    ax1.set_yticklabels(['0.0', '0.25', '0.5', '0.75', '1.0'])
    plt.setp(ax1.yaxis.get_majorticklabels(), rotation = 75)

    ax1.set_xlabel("Fwd. entry size, $|F|$")
    ax1.set_ylabel("CDF", labelpad = -2.5)
    ...
    ax2.set_yticklabels(['0.9', '0.925', '0.95', '0.975', '1.0'])
    plt.setp(ax2.yaxis.get_majorticklabels(), rotation = 70)

    ax2.set_xlabel("FP size, $|FP|$")
    ax2.set_ylabel("CDF", labelpad = -5)
    ax2.set_xticks(np.arange(1, (10 + 1), step = 1))

    ax2.legend(fontsize = FONTSIZE_LEGEND, ncol = 1,
        handletextpad = 0.2, handlelength = 1.0, labelspacing = 0.2, columnspacing = 0.5,
        bbox_to_anchor=(1.050, 0.400))

    fig.subplots_adjust(left = None, bottom = None, right = None, top = None, wspace = 0.4, hspace = None)
    plt.savefig(os.path.join(output_dir, "fp-stats.pdf"), bbox_inches = 'tight', format = 'pdf')
...
if __name__ == "__main__":
    ...
    if not args.data_dir:
        sys.stderr.write("""%s: [ERROR] please supply an input data dir!\n""" % sys.argv[0]) 
        parser.print_help()
        sys.exit(1)
    ...
    if args.case == 'tp-sizes':
        plot_tp_sizes(data, args.output_dir)
    else:
